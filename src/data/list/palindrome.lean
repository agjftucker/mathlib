...
/-- If a list is equal to its reverse, it is a palindrome. -/
theorem of_reverse_eq {l : list α} (h : reverse l = l) : palindrome l :=
begin
...
/-- The inductive and self-reverse definitions are equivalent. -/
theorem iff_reverse_eq {l : list α} : palindrome l ↔ reverse l = l :=
iff.intro to_reverse_eq of_reverse_eq

/-- A list appended to its reverse is a palindrome. -/
theorem of_append_reverse (l : list α) : palindrome (l ++ reverse l) :=
